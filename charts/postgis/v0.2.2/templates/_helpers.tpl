...
{{- $top := .Context -}}
{{- $user := default
	$top.Values.postgresqlUsername
	.User -}}
{{- $port := default
	(int $top.Values.service.port)
	.Port -}}
{{- $db := default
	(include "postgresql.firstDatabaseName" $top)
	.Database -}}
{{- $host := default
	(include "postgresql.service.name" $top)
	 .Host -}}
{{- $password := default
	(include "common.secretValue" $top.Values.postgresqlPassword)
	 .Password -}}
